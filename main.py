...
def logError(msg):
	_, e, _ = sys.exc_info()
	log("Robloxnotif has errored: " + type(e).__name__+" "+msg +"\n"+ str(e), Fore.RED)
	toast.show_toast("Robloxnotif has errored: " + type(e).__name__,msg+" : "+ str(e), threaded=True)
	f3 = open("./error.log","w",encoding="utf-8")
	traceback.print_exc(file=f3)
	f3.close()
try:
	...
	while True:
		try:
			...
			log("Api error: code " + str(data["errors"][0]["code"]) + ": " + data["errors"][0]["message"],Fore.RED)
			toast.show_toast("Api error: code " + str(data["errors"][0]["code"]) + ": " + data["errors"][0]["message"])
